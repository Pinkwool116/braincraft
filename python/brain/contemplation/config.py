...
CONTEMPLATION_CONFIG = {
    ...
    # General conditions
    'conditions': {
        'only_when_idle': True,            # Only contemplate when no active task
        'skip_during_urgent': True,        # Skip if mid-level needs help
        'require_min_experiences': 2,      # Minimum total experiences to start
    },
    ...
    # Frequency settings
    'frequency': {
        'min_interval_seconds': 60,         # Min time between contemplations
        'max_contemplations_per_hour': 10,  # Rate limiting
    }
}
