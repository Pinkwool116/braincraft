...
class MentalState:
    ...
    def __init__(self, shared_state):
        ...
        # Interrupted thought tracking (for contemplation)
        self.interrupted_thought = None
        # Format: {
        #     'mode': contemplation_mode,
        #     'context': partial_context,
        #     'interrupted_at_days': game_days
        # }
    ...
    def save_interrupted_thought(self, mode: str, context: Any):
        """Save contemplation that was interrupted"""
        current_days = self.shared_state.get('agent_age_days', 0)
        
        self.interrupted_thought = {
            'mode': mode,
            'context': context,
            'interrupted_at_days': current_days
        }
        
        logger.info(f"💭 Thought interrupted and saved: {mode}")
    
    def restore_interrupted_thought(self) -> Optional[Dict]:
        """Restore and clear interrupted thought"""
        thought = self.interrupted_thought
        self.interrupted_thought = None
        return thought
    
    async def is_busy(self) -> bool:
        """
        Check if currently mentally busy (should skip contemplation)
        
        Returns True if high-level brain is currently:
        - Processing a modification request from mid-level brain
        - Being asked for guidance/plan modification
        
        This prevents contemplation from interfering with urgent requests.
        
        Note: Experience summarization is done automatically during contemplation,
        not as a separate requested task.
        """
        # Check if mid-level brain has sent a modification request
        mod_request = await self.shared_state.get('modification_request')
        if mod_request and not mod_request.get('processed', False):
            return True  # Mid-level needs immediate attention
        
        return False
    
    def needs_contemplation(self) -> bool:
        """Check if could benefit from contemplation time"""
        return (
            len(self.focus['pending_reflections']) > 5
        )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            'mood': self.mood,
            'focus': self.focus,
            'interrupted_thought': self.interrupted_thought
        }
    ...
    def from_dict(self, data: Dict):
        ...
        self.interrupted_thought = data.get('interrupted_thought', None)
