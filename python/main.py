...
from brain.brain_coordinator import BrainCoordinator
    ...
    try:
        ...
        if 'high_level_brain' in config:
            if 'system_prompt_file' not in config['high_level_brain']:
                config['high_level_brain']['system_prompt_file'] = 'python/prompts/high_level_planning_prompt.txt'
        
        if 'mid_level_brain' in config:
            if 'system_prompt_file' not in config['mid_level_brain']:
                config['mid_level_brain']['system_prompt_file'] = 'python/prompts/mid_level_execution_prompt.txt'
...
async def main():
    ...
    try:
        ...
        logger.info(f"Agent: {config.get('agent_name', 'MindCraft Agent')}")
