...
// Load configuration from project root (2 levels up from python/bridge)
    ...
    async loadPlaytime() {
        ...
        try {
            ...
            if (fs.existsSync(playtimeFile)) {
                ...
                console.log(`Loaded playtime: ${this.totalTicksPlayed} ticks (${ageDays} game days)`);
            } else {
                // First spawn - start from zero
                this.totalTicksPlayed = 0;
                console.log(`First spawn! Starting playtime tracking...`);
            }
        } catch (error) {
        ...
        this.bot.once('spawn', async () => {
            ...
            this.playtimeInterval = setInterval(() => {
                ...
                if (ticksElapsed > 0 && ticksElapsed < 150) {
                    ...
                    // Save every minute (~1200 ticks)
                    if (Math.floor(this.totalTicksPlayed / 1200) !== Math.floor((this.totalTicksPlayed - ticksElapsed) / 1200)) {
                        this.savePlaytime();
                        const ageDays = Math.floor(this.totalTicksPlayed / 24000);
