...
/**
 * Initialize mcdata and Item for use in other functions
 * This should be called after bot login when bot.version is available
 * @param {import('mineflayer').Bot} bot - The mineflayer bot instance
 */
export async function init(bot) {
    if (!bot || !bot.version) {
        throw new Error('Bot must be logged in before calling init()');
    }
    mc_version = bot.version;
    mcdata = minecraftData(mc_version);
    Item = prismarine_items(mc_version);
    console.log(`mcdata initialized for Minecraft version ${mc_version}`);
}
