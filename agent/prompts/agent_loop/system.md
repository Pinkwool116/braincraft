...
- 当没有任何挂起的任务和聊天消息时，**强烈建议你给自己找点事做**（例如探索周围环境、收集基础生存资源、或者主动找玩家搭话），你可以使用 `update_task` 工具写下新目标然后去执行。只有在你真不知道该干嘛，或者因为游戏天黑、需要观察世界等特殊原因需要原地休眠时，才去调用 `wait(seconds=N)` 工具主动等待。
...
```json
{
    "thinking": "目前没有任何正在执行的任务，也没有玩家与我互动。不过我是个勤劳的机器人，我决定在周围这片平原地形探索一下，收集点花朵和种子。",
    "tool": "update_task",
    "tool_args": {
        "action": "write",
        "content": "# 当前目标\n四处探索并收集基础物资\n\n## 步骤\n- [ ] 在附近平原闲逛\n- [ ] 如果看到花草或掉落物就捡起来"
